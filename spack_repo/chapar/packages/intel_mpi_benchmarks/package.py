...
class IntelMpiBenchmarks(MakefilePackage):
    ...
    # Darwin does not provide Linux's malloc.h and requires libgen.h for basename.
    patch("darwin-portability.patch", when="@2021.7 platform=darwin")
    ...
    def edit(self, spec, prefix):
        if spec.satisfies("platform=darwin ^openmpi"):
            env["OMPI_CC"] = self.compiler.cc
            env["OMPI_CXX"] = self.compiler.cxx
