...
- **Vlad container delivery:** Vlad ships as Pyxis images. The primary deliverable is the digest-locked NVIDIA HPC-benchmarks 26.02 base with the promoted vlad release injected, built for both architectures (`linux-x86_64-v4` and `linux-aarch64-gb300`); a secondary NeMo base slot exists fail-closed. Never pull a base image by tag — bases resolve to per-platform descriptor digests from `containers/envs/vlad/image/sources-lock.json`, and builds must refuse to run while that lock is `blocked`. Injection places Spack prefixes at their build-time absolute paths and exposes modules opt-in via `module use`; the base image's own MPI/HPL stack stays first on PATH. See `agents/skills/chapar-vlad-image/SKILL.md`.
...
| `containers/envs/vlad/image/` | Vlad Pyxis image pipeline: build-image.sh, preflight.sh, targets.json, sources-lock.json, site contract, tests |
